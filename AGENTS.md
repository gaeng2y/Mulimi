...
- After modifying code, run `graphify update .` with the isolated `GRAPHIFY_OUT` procedure in `Docs/harness-engineering.md#graphify-artifacts` to keep shared artifacts current without local session history (AST-only, no API cost).
